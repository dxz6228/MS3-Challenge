 ...
 * This class takes a csv file and parses it, inserting the correctly formulated lines from the file into the SQLite
 * database in-memory, and dumping malformed lines into a bad data csv file.
 */
public class csvParser {
    ...
    private ArrayList<String> splitPreserveQuot(String toSplit){
    ...
    public void parseCsv(String csvFile) throws FileNotFoundException, UnsupportedEncodingException, java.sql.SQLException {
        int recReceived=0;
        int recSuccess=0;
        int recFail=0;
        ...
        String firstLine = sc.next(); //to get the columns
        //System.out.println(firstLine);
       // System.out.println(this.splitPreserveQuot(firstLine));
        ArrayList<String> firstSplit = this.splitPreserveQuot(firstLine); //check number of columns and split names off
        ...
        while (sc.hasNext()){
            ...
            if(line.length()>0){
                ...
                ArrayList<String> splitLine = this.splitPreserveQuot(line);
                boolean valid=true;
